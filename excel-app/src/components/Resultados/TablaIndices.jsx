      ...
      <div style={{ padding: '15px', backgroundColor: 'var(--bg-card)', borderRadius: '8px', border: '1px solid var(--border-color)', marginTop: '15px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px' }}>
          ...
          <button 
            ...
            onClick={() => {
              if (esSimple) {
                const datosExport = resultado.detalles.map((fila, idx) => ({
                  "Ítem": fila.item,
                  [tipo === "precios" ? "Precio Base (P₀)" : "Cantidad Base (Q₀)"]: tipo === "precios" ? fila.P0 : fila.Q0,
                  [tipo === "precios" ? "Precio Actual (Pt)" : "Cantidad Actual (Qt)"]: tipo === "precios" ? fila.Pt : fila.Qt,
                  "Relativo Simple (%)": `${fila.relativo.toFixed(2)}%`
                }));
                copiarTablaAExcel(datosExport, `indices_simples_${tipo}`);
              } else {
                copiarTablaAExcel(resultado.detalles, "indices_compuestos");
              }
            }}
        ...
        {esSimple ? (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '15px', marginBottom: '20px' }}>
            <div style={{ padding: '15px', backgroundColor: 'rgba(25, 118, 210, 0.1)', border: '1px solid #1976d2', borderRadius: '8px', textAlign: 'center' }}>
              <h4 style={{ margin: '0 0 5px 0', color: '#1976d2' }}>Índice Agregativo Simple</h4>
        ...
        <button onClick={() => setMostrarDetalles(!mostrarDetalles)} style={{ width: "100%", padding: "10px", backgroundColor: "var(--bg-secondary)", color: "var(--text-color)", border: "1px solid var(--border-color)", borderRadius: "4px", cursor: "pointer", fontWeight: "bold" }}>
          {mostrarDetalles ? "Ocultar Tabla de Cálculos" : "Ver Tabla de Pasos"}
        </button>
