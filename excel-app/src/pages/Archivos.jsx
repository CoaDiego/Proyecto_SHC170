...
export default function Archivos({ usuario }) {
  ...
  const loadFiles = async () => {
    if (!usuario) return; 
    try {
      // 🚧 Aquí luego separaremos la llamada: api.obtenerArchivosPersonales vs api.obtenerArchivosCurso
  ...
  useEffect(() => { 
    loadFiles(); 
    setSelectedFile(null); // Limpiamos la vista al cambiar de usuario
  }, [usuario, tabActiva]);
  ...
  const handleUploadFile = async (fileObj) => {
    ...
    if (tabActiva === 'cursos' && !cursoSeleccionado) {
      alerta.error("Faltan datos", "Selecciona el curso al que quieres subir el material.");
      return;
    }
    ...
    formData.append("visibilidad", tabActiva === 'cursos' ? "privado" : "personal");
  ...
  const handleDeleteFile = async (filename) => {
    ...
    if (tabActiva === 'cursos' && usuario.rol === 'Estudiante') {
      alerta.error("Acceso Denegado", "No puedes eliminar el material subido por el docente.");
      return;
    }
  ...
  return (
    <div className="page-container" style={{ padding: '20px', maxWidth: '1400px', margin: '0 auto' }}>
      ...
      <div style={{ borderBottom: '2px solid var(--border-color)', paddingBottom: '10px', marginBottom: '20px' }}>
        <h2 style={{ margin: 0, color: 'var(--text-main)' }}>Gestión de Datos y Archivos</h2>
        <p style={{ margin: '5px 0 0 0', color: 'var(--text-muted)' }}>
          Espacio de trabajo de: <strong>{usuario?.nombre}</strong> ({usuario?.rol})
        </p>
      </div>

      <div className="files-layout" style={{ display: 'flex', gap: '20px', alignItems: 'flex-start' }}>
        
        {/* ========================================================= */}
        {/* COLUMNA IZQUIERDA: Pestañas y Gestión (35%)               */}
        {/* ========================================================= */}
        <div style={{ flex: '0 0 35%', display: 'flex', flexDirection: 'column', gap: '15px' }}>
          
          {/* 🆕 SELECTOR DE PESTAÑAS */}
          <div style={{ display: 'flex', background: 'var(--bg-card)', borderRadius: '8px', padding: '5px', border: '1px solid var(--border-color)' }}>
            <button 
              onClick={() => setTabActiva('personales')}
              style={{ flex: 1, padding: '10px', border: 'none', borderRadius: '6px', cursor: 'pointer', fontWeight: 'bold',
                       background: tabActiva === 'personales' ? 'var(--accent-color)' : 'transparent',
                       color: tabActiva === 'personales' ? 'white' : 'var(--text-muted)' }}
            >
              👤 Mi Espacio
            </button>
            <button 
              ...
              style={{ flex: 1, padding: '10px', border: 'none', borderRadius: '6px', cursor: 'pointer', fontWeight: 'bold',
                       background: tabActiva === 'cursos' ? 'var(--primary-color)' : 'transparent',
          ...
          <div style={{ background: 'var(--bg-card)', padding: '20px', borderRadius: '8px', border: '1px solid var(--border-color)' }}>
            ...
            {tabActiva === 'cursos' && (
              <div style={{ marginBottom: '20px' }}>
                <label style={{ display: 'block', marginBottom: '8px', fontWeight: 'bold' }}>Selecciona un Curso:</label>
                <select 
                  value={cursoSeleccionado} 
                  onChange={(e) => setCursoSeleccionado(e.target.value)}
                  style={{ width: '100%', padding: '10px', borderRadius: '4px', border: '1px solid #ccc', backgroundColor: '#f8f9fa' }}
                >
                  <option value="">-- Elige un curso para ver material --</option>
            ...
            {!(tabActiva === 'cursos' && usuario?.rol === 'Estudiante') && (
              <div style={{ marginBottom: '20px', paddingBottom: '20px', borderBottom: '1px solid var(--border-color)' }}>
                <h3 style={{ margin: '0 0 10px 0', fontSize: '1.1rem' }}>
                  {tabActiva === 'cursos' ? 'Subir Material al Curso' : 'Subir Archivo Personal'}
                </h3>
            ...
            {tabActiva === 'cursos' && !cursoSeleccionado ? (
              <p style={{ color: 'var(--text-muted)', fontStyle: 'italic', textAlign: 'center' }}>Selecciona un curso arriba para ver sus archivos.</p>
            ) : (
              <ExcelViewer
                files={files}
        ...
        {/* ========================================================= */}
        <div style={{ flex: '1', background: 'var(--bg-card)', padding: '20px', borderRadius: '8px', border: '1px solid var(--border-color)', minHeight: '600px' }}>
          <h3 style={{ margin: '0 0 15px 0', fontSize: '1.2rem', borderBottom: '1px solid var(--border-color)', paddingBottom: '10px' }}>
          ...
          {selectedFile ? (
            ...
            <div style={{ display: 'flex', height: '80%', alignItems: 'center', justifyContent: 'center', color: 'var(--text-muted)', fontStyle: 'italic', backgroundColor: 'var(--bg-main)', borderRadius: '8px', border: '1px dashed #ccc' }}>
