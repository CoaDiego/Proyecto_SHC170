...
export default function Grupos() {
  ...
  // 1. BASE DE DATOS GLOBAL DE CURSOS (Lo que ven/crean los docentes)
  const [misCursos, setMisCursos] = useState([
    { id: "EST-101", nombre: "Estadística Empresarial I", codigo: "EST-101", alumnos: 24, archivos: 3 },
    { id: "DAT-205", nombre: "Análisis de Datos Avanzado", codigo: "DAT-205", alumnos: 15, archivos: 5 }
  ]);
  ...
  const handleCrearCurso = (e) => {
    ...
    if (!nuevoNombre.trim() || !nuevoCodigo.trim()) {
      alerta.error("Campos vacíos", "Por favor ingresa el nombre y la sigla del curso.");
      return;
    }
  ...
  const handleUnirseCurso = () => {
    ...
    if (!cursoEncontrado) {
      alerta.error("Curso no encontrado", "Asegúrate de haber escrito bien el código. (Ej: EST-101)");
      return;
    }

    // 2. Verificamos si el estudiante ya está inscrito
    const yaEstaInscrito = cursosInscritos.find(c => c.codigo === codigoLimpiado);
    ...
    alerta.success("¡Inscripción Exitosa!", `Te has unido a: ${cursoEncontrado.nombre}`);
  };
    ...
    <div style={{ padding: '30px', maxWidth: '1200px', margin: '0 auto', position: 'relative' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '20px' }}>
            ...
            {misCursos.map(curso => (
              <div key={curso.id} style={{ background: 'var(--bg-card, white)', padding: '20px', borderRadius: '8px', border: '1px solid var(--border-color, #eee)', boxShadow: '0 4px 6px rgba(0,0,0,0.05)' }}>
                ...
                <p style={{ margin: '0 0 5px 0', color: 'var(--text-muted, #666)' }}><strong>Código de Matriculación:</strong> {curso.codigo}</p>
                <p style={{ margin: '0 0 15px 0', color: 'var(--text-muted, #666)' }}>👥 {curso.alumnos} Estudiantes | 📁 {curso.archivos} Archivos</p>
                
                <div style={{ display: 'flex', gap: '10px' }}>
                  <button style={{ flex: 1, padding: '8px', background: 'var(--bg-main, #f4f4f4)', border: '1px solid var(--border-color, #ccc)', borderRadius: '4px', cursor: 'pointer', fontWeight: 'bold' }}>
                    Gestionar
                  </button>
                  <button onClick={() => navigate('/archivos')} style={{ flex: 1, padding: '8px', background: 'var(--primary-color, #2c3e50)', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontWeight: 'bold' }}>
                    Subir Material
                  </button>
                </div>
              </div>
          ...
          <div style={{ background: 'var(--bg-card, white)', padding: '20px', borderRadius: '8px', border: '1px solid var(--border-color, #eee)', marginBottom: '30px' }}>
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              <input 
                ...
                style={{ padding: '10px', flex: 1, borderRadius: '5px', border: '1px solid #ccc', textTransform: 'uppercase' }}
              />
              <button 
                onClick={handleUnirseCurso}
                style={{ background: '#27ae60', color: 'white', padding: '10px 20px', border: 'none', borderRadius: '5px', cursor: 'pointer', fontWeight: 'bold' }}
              >
                Unirse al Curso
              </button>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '20px' }}>
              ...
              {cursosInscritos.map(curso => (
                <div key={curso.id} style={{ background: 'var(--bg-card, white)', padding: '20px', borderRadius: '8px', border: '1px solid var(--border-color, #eee)', borderTop: '4px solid #27ae60' }}>
                  <h3 style={{ margin: '0 0 10px 0', color: 'var(--text-main, #333)' }}>{curso.nombre}</h3>
                  <p style={{ margin: '0 0 15px 0', color: 'var(--text-muted, #666)' }}><strong>Código:</strong> {curso.codigo}</p>
                  <button onClick={() => navigate('/archivos')} style={{ width: '100%', padding: '10px', background: 'white', border: '1px solid #27ae60', color: '#27ae60', borderRadius: '4px', cursor: 'pointer', fontWeight: 'bold', transition: 'all 0.3s' }}>
      ...
      {mostrarModal && (
        <div style={{
          ...
          <div style={{
            background: 'white', padding: '30px', borderRadius: '10px', width: '400px',
            boxShadow: '0 10px 25px rgba(0,0,0,0.2)'
          }}>
            <h2 style={{ marginTop: 0, color: 'var(--primary-color)' }}>Crear Nuevo Curso</h2>
            <p style={{ color: '#666', fontSize: '0.9rem', marginBottom: '20px' }}>Configura el espacio de trabajo para tus estudiantes.</p>
            
            <form onSubmit={handleCrearCurso}>
              <div style={{ marginBottom: '15px' }}>
                <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold' }}>Nombre de la Materia:</label>
                <input 
                  type="text" 
                  value={nuevoNombre}
              ...
              <div style={{ marginBottom: '25px' }}>
                <label style={{ display: 'block', marginBottom: '5px', fontWeight: 'bold' }}>Sigla / Código de Matriculación:</label>
                <input 
                  type="text" 
                  value={nuevoCodigo}
                  onChange={(e) => setNuevoCodigo(e.target.value.toUpperCase())}
                  placeholder="Ej. ADM-300"
                  style={{ width: '100%', padding: '10px', borderRadius: '5px', border: '1px solid #ccc', boxSizing: 'border-box', textTransform: 'uppercase' }}
                />
              </div>
              ...
              <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end' }}>
                <button 
                  ...
                  style={{ padding: '10px 15px', background: '#eee', color: '#333', border: 'none', borderRadius: '5px', cursor: 'pointer', fontWeight: 'bold' }}
                >
                  Cancelar
                </button>
                <button 
                  type="submit"
                  style={{ padding: '10px 20px', background: 'var(--accent-color)', color: 'white', border: 'none', borderRadius: '5px', cursor: 'pointer', fontWeight: 'bold' }}
                >
                  Guardar Curso
                </button>
              </div>
