...
export function useCalculadoraExcel(filename, sheet) {
  ...
  const calcularFractiles = (datos, kPerc) => {
    ...
    const resultados = [];
    for(let i=1; i<=3; i++) resultados.push({ Tipo: "Cuartil", Símbolo: `Q${i}`, Valor: calcFractil(i/4) });
    for(let i=1; i<=9; i++) resultados.push({ Tipo: "Decil", Símbolo: `D${i}`, Valor: calcFractil(i/10) });

    const k = Number(kPerc);
    if(k > 0 && k < 100) {
      resultados.push({ Tipo: "Percentil", Símbolo: `P${k}`, Valor: calcFractil(k/100) });
    }
    return resultados;
  };
