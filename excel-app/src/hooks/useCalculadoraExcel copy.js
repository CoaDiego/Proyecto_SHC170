...
// 👇 IMPORTAMOS EL NUEVO MOTOR DE REGRESIÓN
...
export function useCalculadoraExcel(filename, sheet) {
  ...
  // 👇 NUEVO ESTADO PARA EL TEMA 6
  const [tipoRegresion, setTipoRegresion] = useState("lineal");
  ...
  const ejecutarCalculo = () => {
    ...
    // 👇 NUEVO PASE VIP: ANÁLISIS DE REGRESIÓN
    if (calculo === "regresion_simple") {
      if (!selectedColumn || !selectedColumnY) return;
      // Para regresión, obligamos a que ambas columnas sean números y limpiamos los vacíos
      const dataX = [];
      const dataY = [];
      ...
      const resRegresion = RegMath.calcularRegresionSimple(dataX, dataY, tipoRegresion);
      ...
      if (!resRegresion) {
        ...
        setErrorNumerico(false);
        setResultado({ tipo: "regresion", ...resRegresion });
      }
      return;
    }
  ...
  useEffect(() => {
    ...
    // 👇 NOTA: Excluimos 'excelData' a propósito para que no recalcule si escribes a mano.
    // Añadimos 'tipoRegresion' para que reaccione al cambiar de Lineal a Exponencial
  ...
  return {
    ...
    percentilK,
    setPercentilK,
    tipoRegresion, // <-- Exportamos la variable de regresión
    setTipoRegresion, // <-- Exportamos el modificador
    setSelectedColumn,
    setCalculo,
    setTipoIntervalo,
    setMetodoK,
